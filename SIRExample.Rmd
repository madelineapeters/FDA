...
Now we will generate some data with noise drawn from a normal distribution with mean zero and standard deviation 200. We will also replace all negative values with zero and round so the data are counts.
```{r}
...
for (ilam in 1:length(lambdafac)){
  t.res = Profile.LS(sir.fun,SIRdata,SIRtimes,temp.pars,temp.coefs,SIRbasis,lambdafac[ilam],active=activepars)
